...
func calculateMinimumHP(dungeon [][]int) int {
	hei := len(dungeon)
	wid := len(dungeon[0])
	maxMin := math.MinInt64

	minForCur := map[[3]int]int{}
	curForMin := map[[3]int]int{}

	var search func([2]int, int, int)
	search = func(pos [2]int, curHealth int, minHealth int) {
		curHealth += dungeon[pos[0]][pos[1]]
		...
		if v, ok := minForCur[[3]int{pos[0], pos[1], curHealth}]; ok && v >= minHealth {
			return
		}
		minForCur[[3]int{pos[0], pos[1], curHealth}] = minHealth
		if v, ok := curForMin[[3]int{pos[0], pos[1], minHealth}]; ok && v >= curHealth {
			return
		}
		curForMin[[3]int{pos[0], pos[1], minHealth}] = curHealth

		if pos[0] == hei-1 && pos[1] == wid-1 {
			if minHealth > maxMin {
				maxMin = minHealth
			}
			return
		...
		if pos[0] < hei-1 && pos[1] < wid-1 {
			// Take the highest path first which should increase short-circuiting
			if dungeon[pos[0]+1][pos[1]] >= dungeon[pos[0]][pos[1]+1] {
				search([2]int{pos[0] + 1, pos[1]}, curHealth, minHealth)
				search([2]int{pos[0], pos[1] + 1}, curHealth, minHealth)
			} else {
				search([2]int{pos[0], pos[1] + 1}, curHealth, minHealth)
				search([2]int{pos[0] + 1, pos[1]}, curHealth, minHealth)
			}
