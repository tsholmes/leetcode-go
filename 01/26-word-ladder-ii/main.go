...
func findLadders(beginWord string, endWord string, wordList []string) [][]string {
	...
	check := func(a, b string) {
		...
		if count == 1 {
			if _, ok := edges[a]; !ok {
				edges[a] = map[string]struct{}{}
			}
			edges[a][b] = struct{}{}
			if _, ok := edges[b]; !ok {
				edges[b] = map[string]struct{}{}
			}
			edges[b][a] = struct{}{}
		}
	}
	for i := 0; i < len(wordList); i++ {
		check(beginWord, wordList[i])
		for j := i + 1; j < len(wordList); j++ {
			check(wordList[i], wordList[j])
		}
	}

	if _, ok := edges[endWord]; !ok {
		return nil
	}
	...
	seen := map[string]int{}
	for len(work) > 0 && (len(res) == 0 || len(res[0]) == len(work[0])) {
		l := work[0]
		work = work[1:]
		...
		if s, ok := seen[w]; ok && s < len(l) {
			continue
		}
		seen[w] = len(l)

		for w2 := range edges[w] {
			v := true
			for _, ww := range l {
				if ww == w2 {
					v = false
			...
			work = append(work, append(append([]string{}, l...), w2))
		}
	}
