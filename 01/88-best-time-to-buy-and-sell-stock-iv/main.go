...
func maxProfit(k int, prices []int) int {
	...
	for i := 0; i < len(prices); i++ {
		for stage := k * 2; stage > 0; stage-- {
			if stage > i+1 {
				continue
			}
			if dp[stage-1] == math.MinInt32 {
				continue
			}
			if stage%2 == 0 {
				np := dp[stage-1] + prices[i]
				if np > dp[stage] {
					dp[stage] = np
				}
			} else {
