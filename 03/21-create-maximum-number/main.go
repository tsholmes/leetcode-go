...
func maxNumber(nums1 []int, nums2 []int, k int) []int {
	maxK := func(nums []int, k int) []int {
		var res []int
		i := 0
		for len(res) < k {
			maxd := -1
			maxi := -1
			for j := i; j < len(nums) && len(nums)-j+len(res) >= k; j++ {
				if nums[j] > maxd {
					maxd = nums[j]
					maxi = j
				}
	...
	for kk := 0; kk <= k; kk++ {
		...
		n1, n2 := maxK(nums1, k1), maxK(nums2, k2)

		nums = nums[:0]
		i, j := 0, 0
		for i < len(n1) && j < len(n2) {
			if gt(n1[i:], n2[j:]) {
				nums = append(nums, n1[i])
				i++
			} else {
				nums = append(nums, n2[j])
				j++
			}
		}
		nums = append(nums, n1[i:]...)
